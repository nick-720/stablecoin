...
use crate::{Config, SEED_CONFIG_ACCOUNT, SEED_COLLATERAL_ACCOUNT, SEED_SOL_ACCOUNT, Collateral, deposit_sol, mint_tokens, check_health_factor};
...
pub struct DepositCollateralAndMintTokens<'info> {
    ...
    pub config_account: Box<Account<'info, Config>>, // holds global config info for algo stablecoin

    #[account(mut)]
    pub mint_account: InterfaceAccount<'info, Mint>, // the mint for the stablecoin
    ...
    pub collateral_account: Account<'info, Collateral>, // holds collateral info for this specific depositor
    ...
    pub sol_account: SystemAccount<'info>, // holds user's deposited SOL collateral
    ...
    pub token_account: InterfaceAccount<'info, TokenAccount>, // user's token account to receive minted stablecoins
...
pub fn process_deposit_collateral_and_mint_tokens(
    ctx: Context<DepositCollateralAndMintTokens>, amount_collateral: u64, amount_to_mint: u64
) -> Result<()> {
